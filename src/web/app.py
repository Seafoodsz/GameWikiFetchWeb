...
def load_default_config():
    ...
    if os.path.exists(env_path):
        try:
                ...
                for line in f:
                    ...
                    try:
                        ...
                        if key == 'wiki_url':
                            ...
                            config['download_images'] = value if isinstance(value, bool) else value.lower() == 'true'
                        elif key == 'download_tables':
                            config['download_tables'] = value if isinstance(value, bool) else value.lower() == 'true'
                        elif key == 'user_agent':
                            ...
                            config['save_html'] = value if isinstance(value, bool) else value.lower() == 'true'
                        elif key == 'log_level':
            ...
            logging.info(f"已从.env文件加载配置: {env_path}")
        except Exception as e:
            logging.error(f"加载.env文件失败: {str(e)}")
    
    # 设置默认值
    if 'wiki_url' not in config:
        config['wiki_url'] = ''
    if 'output_dir' not in config:
        config['output_dir'] = os.path.join(root_dir, 'wiki_data', 'default')
        logging.info(f"未设置输出目录，使用默认目录: {config['output_dir']}")
    if 'max_depth' not in config:
        config['max_depth'] = 2
    if 'download_images' not in config:
        config['download_images'] = True
    if 'download_tables' not in config:
        config['download_tables'] = True
...
def start_fetcher_internal(form):
    ...
    if not config['output_dir']:
        ...
        # 创建输出目录
        config['output_dir'] = os.path.join(root_dir, 'wiki_data', wiki_name)
        logging.info(f"自动创建输出目录: {config['output_dir']}")
    elif not os.path.isabs(config['output_dir']):
        # 如果是相对路径，转换为绝对路径
        config['output_dir'] = os.path.join(root_dir, config['output_dir'])
        logging.info(f"转换为绝对路径: {config['output_dir']}")
...
def organize_content():
    ...
    if not output_dir:
        # 如果没有设置输出目录，使用默认目录
        output_dir = os.path.join(root_dir, 'wiki_data', 'default')
        logging.info(f"未设置输出目录，使用默认目录: {output_dir}")
...
def download_file(filename):
    ...
    # 确保输出目录存在
    if not os.path.exists(output_dir):
        flash(f'输出目录不存在: {output_dir}', 'danger')
        return redirect(url_for('index'))
    
    # 检查文件是否存在
    file_path = os.path.join(output_dir, filename)
    if not os.path.exists(file_path):
        flash(f'文件不存在: {filename}', 'danger')
        return redirect(url_for('index'))
...
def view_index():
    ...
    return send_from_directory(organized_dir, 'index.html')
...
""".format(**config)
    ...
    try:
        with open(env_path, 'w', encoding='utf-8') as f:
            f.write(env_content)
        logging.info(f"配置已保存到: {env_path}")
    except Exception as e:
        logging.error(f"保存配置文件失败: {str(e)}")
        flash(f"保存配置文件失败: {str(e)}", 'danger')
